<script lang="ts">
  ...
  const handleClick = (event: MouseEvent | KeyboardEvent): void => {
    ...
    rotation =
      Math.round(
        ((params.l / 2) * params.rad -
          params.rad * params.res -
          params.offsetRad -
          params.rad) *
          360
      ) +
  ...
  const createTwistingPie = (items: string[]) => {
    ...
    const colors = scaleOrdinal().range(rColors);

    const radius = Math.min(svgWidth, svgHeight) / 2;

    const pie = arc().innerRadius(0).outerRadius(radius);
...
<div class="background">
  <div class="pie-wrapper" on:click={handleClick} on:keydown={handleClick}>
        ...
        {#each angles as value, i}
          {#if lots[i].indexOf("://") === -1}
            <text
              ...
              transform={`rotate(${
                (angles[i] + params.offset) * (180 / Math.PI)
              })`}
              y="13%"
...
<style>
  ...
  text {
    font-family: Arial, Helvetica, sans-serif, sans-serif-condensed, sans-serif;
